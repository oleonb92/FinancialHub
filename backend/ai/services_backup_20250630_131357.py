...
class AIService:
    ...
    def get_high_quality_prediction(self, model_name: str, data: Any, 
        ...
        try:
            ...
            # Si ni el fallback cumple, usar ChatGPT como backup_llm
            if prediction_type == 'classification' and model_name == 'transaction_classifier':
                logger.info(f"[AI][LLM] Usando ChatGPT como backup_llm para transacción {getattr(data, 'id', None)}")
                from .services import get_category_suggestion_from_gpt
                gpt_suggestion = get_category_suggestion_from_gpt(data)
                if gpt_suggestion:
                    logger.info(f"[AI][LLM] Retornando predicción BACKUP_LLM para transacción {getattr(data, 'id', None)}: '{gpt_suggestion}'")
                    return {
                        'prediction': {
                            'category_id': None,
                            'category_name': gpt_suggestion,
                            'confidence': 0.99,
                            'model_name': 'backup_llm',
                        },
                        'quality_status': 'backup_llm',
                        'model_used': 'backup_llm',
                        'accuracy': 1.0,
                        'confidence': 0.99,
                        'fallback_reason': 'all_models_failed_confidence_threshold',
                        'category_name': gpt_suggestion,  # Asegurar que se incluya el nombre
                    }
                else:
                    logger.warning(f"[AI][LLM] ChatGPT no devolvió ninguna sugerencia para transacción {getattr(data, 'id', None)}")
    ...
    def analyze_transaction(self, transaction):
        ...
        try:
            ...
            # 7. VERIFICAR CALIDAD CON QUALITY GATE
        ...
        try:
            ...
            if transaction_data:
                ...
                self._evaluate_models(transaction_data)
                    ...
                    logger.info(f"[AI][TRAINING] Entrenamiento completado exitosamente con {len(transaction_data)} transacciones")
    ...
    def _get_cached_analysis_result(self, transaction: Transaction) -> Dict[str, Any]:
        """Obtiene el resultado del análisis cacheado de una transacción."""
        # Verificar si es sugerencia de LLM
        is_llm_suggestion = transaction.ai_notes and "[backup_llm]" in transaction.ai_notes
        
        # Determinar si la categoría actual es correcta
        current_category_id = transaction.category.id if transaction.category else None
        suggested_category_id = transaction.ai_category_suggestion.id if transaction.ai_category_suggestion else None
        is_correct = current_category_id == suggested_category_id and suggested_category_id is not None
        
        # Generar mensaje moderno
        if is_llm_suggestion:
            modern_message = f"🤖 Sugerencia de ChatGPT: '{transaction.ai_category_suggestion.name if transaction.ai_category_suggestion else 'N/A'}' (alta confianza)"
        elif transaction.ai_confidence and transaction.ai_confidence >= 0.85:
            modern_message = f"✅ ¡Todo bien! La categoría asignada es la más adecuada según la IA."
        else:
            modern_message = f"⚠️ Sugerencia provisional: '{transaction.ai_category_suggestion.name if transaction.ai_category_suggestion else 'N/A'}' (baja confianza)"
        
        result = {
            'transaction_id': transaction.id,
            'classification': {
                'category_id': transaction.ai_category_suggestion.id if transaction.ai_category_suggestion else None,
                'category_name': transaction.ai_category_suggestion.name if transaction.ai_category_suggestion else None,
                'confidence': transaction.ai_confidence,
                'quality_status': 'backup_llm' if is_llm_suggestion else 'cached',
                'model_used': 'backup_llm' if is_llm_suggestion else 'cached',
                'is_correct': is_correct,
                'modern_message': modern_message
            },
            'behavior_analysis': {
                'pattern_score': 0,
            ...
            'insights': [transaction.ai_notes] if transaction.ai_notes else [],
            'risk_score': 0,
            'overall_quality': 'backup_llm' if is_llm_suggestion else 'cached',
            'quality_warnings': [],
            'suggestion': {
                'status': 'already_approved' if is_correct else 'needs_update',
                'message': 'Análisis cacheado de alta calidad',
                'needs_update': not is_correct and suggested_category_id is not None,
                'current_category_id': current_category_id,
                'suggested_category_id': suggested_category_id,
                'already_approved': is_correct
            }
        }
        return result
    ...
    def _check_suggestion_approved(self, transaction: Transaction, suggested_category_id: int) -> bool:
        ...
        try:
            current_category_id = transaction.category.id if transaction.category else None
            return current_category_id == suggested_category_id
            except Exception as e:
...
def get_category_suggestion_from_gpt(transaction):
    """
    Usa OpenAI GPT para sugerir una categoría para una transacción, usando SIEMPRE la lista en inglés.
    """
    import os
    api_key = os.getenv('OPENAI_API_KEY', getattr(settings, 'OPENAI_API_KEY', None))
    if not api_key:
        logger.error("[AI][LLM] OPENAI_API_KEY no está configurada")
        return None
    openai.api_key = api_key
    model = os.getenv('AI_MODEL', getattr(settings, 'AI_MODEL', 'gpt-4'))
    temperature = float(os.getenv('AI_TEMPERATURE', getattr(settings, 'AI_TEMPERATURE', 0.7)))
    max_tokens = int(os.getenv('AI_MAX_TOKENS', getattr(settings, 'AI_MAX_TOKENS', 2000)))
    prompt = build_chatgpt_prompt(transaction)
    try:
        logger.info(f"[AI][LLM] Solicitando sugerencia a ChatGPT para transacción {getattr(transaction, 'id', '')}...")
        ...
        logger.info(f"[AI][LLM] Sugerencia de ChatGPT: '{suggestion}' para transacción {getattr(transaction, 'id', '')}")
        
        # Validar respuesta
        import re
        match = re.match(r"Category:\s*(.+)\nSubcategory:\s*(.*)", suggestion)
        if match:
            category = match.group(1).strip()
            subcategory = match.group(2).strip()
            
            # Validar que la categoría y subcategoría estén en la lista permitida
            if validate_category_and_subcategory(category, subcategory):
                return f"{category} - {subcategory}" if subcategory else category
            else:
                logger.warning(f"[AI][LLM] Sugerencia fuera de lista permitida: Category='{category}', Subcategory='{subcategory}'. Marcando como 'Unclassified'.")
                return "Unclassified"
        
        logger.warning(f"[AI][LLM] Formato de sugerencia inválido: '{suggestion}'. Marcando como 'Unclassified'.")
        return "Unclassified"
    except Exception as e:
        logger.error(f"Error al obtener sugerencia de categoría de GPT: {e}")
        return None

def build_chatgpt_prompt(transaction):
    """Construye el prompt para ChatGPT usando ÚNICAMENTE la lista de categorías/subcategorías del archivo categories_en.json"""
    try:
        # Cargar directamente desde el archivo para asegurar que solo use las categorías en inglés
        with open(CATEGORIES_FILE, 'r') as f:
            data = json.load(f)
        ...
        for entry in data:
            if entry['subcategories']:
                cat_lines.append(f"- {entry['category']}: {', '.join(entry['subcategories'])}")
            else:
                cat_lines.append(f"- {entry['category']}")
        
        cat_block = '\n'.join(cat_lines)
        
        logger.info(f"[AI][PROMPT] Generando prompt con {len(data)} categorías desde categories_en.json")
        
        prompt = f"""
You are a financial assistant. Choose the most appropriate category and subcategory for the following transaction, using ONLY the options from the list below. 

IMPORTANT RULES:
1. You MUST choose from the exact categories and subcategories listed below
2. Do NOT invent new categories or translate existing ones
3. Do NOT use categories that are not in this list
4. If no category fits perfectly, choose the closest one from the list
5. Respond ONLY with the exact format specified
...
Subcategory: [exact subcategory name from the list, or leave empty if no subcategory applies]
"""
        return prompt
        
    except Exception as e:
        logger.error(f"[AI][PROMPT] Error cargando categorías desde {CATEGORIES_FILE}: {e}")
        # Fallback con categorías básicas en inglés
        fallback_prompt = f"""
You are a financial assistant. Choose the most appropriate category and subcategory for the following transaction, using ONLY the options from the list below. 
...
Available Categories and Subcategories:
- Income: Salary, Freelance Income, Side Hustles, Sales Revenue, Rental Income, Interest Earned, Dividend Income, Refunds, Cash Gifts, Grants, Other Income
- Business Expenses: Advertising & Marketing, Bank & Payment Fees, Car & Vehicle Expenses, Client Meals, Contractor Payments, Education & Courses, Employee Wages, Freelancer Payments, Home Office Expenses, Insurance, Legal & Professional Fees, Licensing & Permits, Office Supplies, Phone & Internet, Software & Subscriptions, Taxes & Licenses, Travel & Transportation, Utilities, Website & Hosting, Other Business Expenses
- Personal Expenses: Groceries, Dining Out, Rent or Mortgage, Utilities, Phone & Internet, Health & Dental, Car Payment, Gas & Fuel, Insurance (Home, Auto), Clothing & Accessories, Gifts & Donations, Subscriptions, Childcare & School, Personal Care, Gym & Fitness, Entertainment, Vacations & Travel, Pet Expenses, Other Personal Expenses
- Assets: Bank Accounts, Cash on Hand, Investments, Property, Vehicle, Business Equipment, Other Assets
- Liabilities: Credit Cards, Student Loans, Personal Loans, Business Loans, Mortgage, Car Loan, Taxes Owed, Other Liabilities
- Savings & Goals: Emergency Fund, Vacation Fund, Down Payment, Retirement, Education Savings, Investment Savings, Other Savings
- Taxes: Federal Taxes, State Taxes, Sales Tax Paid, Self-Employment Tax, Estimated Taxes, Other Tax Payments
...
def validate_category_and_subcategory(category, subcategory):
    """Valida si una categoría y subcategoría están en la lista permitida"""
    try:
        # Recargar las categorías para asegurar que estén actualizadas
        categories, subcategories = load_categories_and_subcategories()
        
        # Verificar si la categoría existe
        if category not in categories:
            logger.warning(f"[AI][VALIDATION] Categoría '{category}' no está en la lista permitida")
            return False
        
        # Si hay subcategoría, verificar que la combinación sea válida
        if subcategory and subcategory.strip():
            if (category, subcategory.strip()) not in subcategories:
                logger.warning(f"[AI][VALIDATION] Subcategoría '{subcategory}' no es válida para categoría '{category}'")
                return False
        
        logger.info(f"[AI][VALIDATION] Categoría '{category}' y subcategoría '{subcategory}' son válidas")
        return True
        
    except Exception as e:
        logger.error(f"[AI][VALIDATION] Error validando categoría '{category}' y subcategoría '{subcategory}': {e}")
        return False
