...
def test_has_pro_access_global():
    ...
    assert has_pro_access(user, feature="org_advanced_reports") is False

@pytest.mark.django_db
def test_has_pro_access_trial():
    user = User.objects.create(
        username="trial_user",
        pro_trial_until=timezone.now() + timedelta(days=5),
        account_type="personal"
    )
    assert has_pro_access(user, feature="org_advanced_reports") is True
    assert has_pro_access(user, feature="multi_org_panel") is False
...
def test_has_pro_access_org_plan():
    ...
    assert has_pro_access(user, org, feature="org_advanced_reports") is False

@pytest.mark.django_db
def test_has_pro_access_feature_list():
    user = User.objects.create(
        username="feature_user",
        pro_features_list=["multi_org_panel"],
        account_type="accountant"
    )
    assert has_pro_access(user, feature="multi_org_panel") is True
    assert has_pro_access(user, feature="org_advanced_reports") is False
...
@pytest.mark.django_db
def test_has_pro_access_member_features():
    user = User.objects.create(
